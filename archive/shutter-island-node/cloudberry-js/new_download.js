...
.then(function () {
    cam.listMedia().then(function (result) {
        ...
        var dateTaken = new Date(lastFile.mod * 1000); /* x1000 timestamp unix > timestamp ms */
dateTaken = dateTaken.toISOString();      
 // dateTaken = dateTaken.toString();
       // dateTaken = dateTaken.split('GMT-0230(NDT)').join('');
       // dateTaken = dateTaken.split('GMT-0230(NST)').join('');
       // dateTaken = dateTaken.split(' ').join('-');

	    newFilePath = shutterBoxPath + "/" + dateTaken + '_' + lastFile.n;
